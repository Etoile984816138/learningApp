<template>
    <transition name="slide-up">
        <footer class="app-bottom-navigator-wrapper" v-show="show && navs.length">
             <slot name="navs">
                <v-bottom-nav :value="show" absolute class="transparent">
                    <v-btn v-for="nav in navs" :key="nav.name" :value="nav.active" @click.native="handleNavClick(nav.route, nav.name)" flat primary>
                        <span>{{ nav.text }}</span>
